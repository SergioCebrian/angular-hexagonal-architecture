...
describe('HttpService', () => {
  ...
  it('[getAll] should return an array', async() => {
    const getAllTasks$ = service.getAll(apiUrlMock);
    const configPromise = firstValueFrom(getAllTasks$);
    const req = httpTesting.expectOne(`${apiUrl}/${apiUrlMock}`, 'Request to load all tasks');
    
    expect(req.request.method).toBe('GET');
    req.flush([mockTask]);
    expect(await configPromise).toEqual([mockTask]);
    httpTesting.verify();
  });

  it('[getOne] should return an object', async() => {
    const getOneTask$ = service.getOne(`${apiUrlMock}/1`);
    const configPromise = firstValueFrom(getOneTask$);
  ...
  it('[create] should return a new object', async() => {
    ...
    const configPromise = firstValueFrom(createTask$);
  ...
  it('[update] should update an object', async() => {
    ...
    const configPromise = firstValueFrom(updateTask$);
